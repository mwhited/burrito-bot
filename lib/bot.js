...
class Bot {
    ...
    static checkMessage(message) {
        ...
        const workoutDiscRegex = /^\/workout\s+disc/;
        const workoutDiscGroupRegex =  /^\/workout\s+disc\s+(\d+)\/(\d+)\s*-\s*([^]*)/;
        ...
        if (messageText && workoutBotRegex.test(messageText)  && message.name!=process.env.BOT_NAME) {
            if (workoutLinkRegex.test(messageText)) {
                ...
                if (workout && workout.length == 4) {
                    ...
                    Workouts.updateWorkoutSheet(message.name, date, workout[3]);
                    return 'workout added for ' + message.name + ' on ' + date + ' with content: ' + workout[3];
                }
                else {
                    this.sendMainChannelMessage("I\'m sorry, I don't understand. Please format your message:/workout log <month#>/<day#> - <workout>");
                    return null;
                }
            }
            else if (workoutDiscRegex.test(messageText)) {
                var discwo = workoutDiscGroupRegex.exec(messageText);
                if(discwo && discwo.length == 4) {
                    let date = discwo[1] + '/' + discwo[2];
                    Workouts.updateDiscSheet(message.name, date, discwo[3]);
                    return 'disc workout added for ' + message.name + ' on ' + date + ' with content: ' + discwo[3];
                }
                else {
                    this.sendMainChannelMessage("I\'m sorry, I don't understand. Please format your message:/workout disc <month#>/<day#> - <disc workout>");
                    return null;
                }
            }
            else{
                this.sendMainChannelMessage("Available commands are: /workout link\n/workout log\n/workout disc");
                return null;
            }
        }
