...
class Bot {
    ...
    static checkMessage(message) {
        ...
        const workoutLogRegex = /\/workout\s+log\s+(\d+)\/(\d+)\s*-\s*([^]*)/;

        // Check if the GroupMe message has content and if the regex pattern is true
        if (messageText && shrugRegex.test(messageText)) {
            // Check is successful, return a message!
            return '¯\\_(ツ)_/¯';
        }

        if (messageText && workoutBotRegex.test(messageText)) {
            if (workoutLinkRegex.test(messageText)) {
                ...
                var workout = workoutLogRegex.exec(messageText);
