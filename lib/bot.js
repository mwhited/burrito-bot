        ...
        if (messageText && workoutBotRegex.test(messageText)  && message.name!=process.env.BOT_NAME) {
            if (workoutLinkRegex.test(messageText)) {
                ...
                if(discwo && discwo.length == 4) {
                    let date = workout[1] + '/' + workout[2];
                    Workouts.updateDiscSheet(message.name, date, workout[3]);
                    return 'disc workout added for ' + message.name + ' on ' + date + ' with content: ' + workout[3];
                }
                else {
                    return "I\'m sorry, I don't understand. Please format your message:/workout disc <month#>/<day#> - <disc workout>";
                }
            }
            else{
